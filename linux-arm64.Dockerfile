...
ARG VERSION
ARG WEB_VERSION
RUN debfile="/tmp/jellyfin.deb" && wget2 -nc -O "${debfile}" "https://repo.jellyfin.org/releases/server/ubuntu/stable-rc/server/jellyfin-server_${VERSION}_arm64.deb" && dpkg --install "${debfile}" && rm "${debfile}" && \
